...
float3 light_pos = float3(2.0f, 0.0f, 3.0f);
...
int main() 
{
    ...
    float umbrella1_vertices[] = {
        -0.25, 0.5, 0.0, 0.0f, 0.0f, 1.0f,
        0.25, 0.5, 0.0, 0.0f, 0.0f, 1.0f,
        0.0, 0.0, 0.0, 0.0f, 0.0f, 1.0f,

        0.5, 0.25, 0.0, 0.0f, 0.0f, 1.0f,
        0.5, -0.25, 0.0, 0.0f, 0.0f, 1.0f,
        0.0, 0.0, 0.0, 0.0f, 0.0f, 1.0f,
    ...
    {   
        ...
        float4x4 view;
        float4x4 projection; 
        float3 view_pos = float3(0.0f, 0.0f, -10.0f);
        ...
        for (unsigned int i = 1; i < 5; i++)
        {
            ...
            float rot_rate = 0.3; 
            float4x4 move = transpose(translate4x4(make_float3(1.5 * cos(cur_time * rot_rate), 0.0, 1.5 * sin(cur_time * rot_rate))));
            ...
            coub_program.SetUniform("light_pos", light_pos);
            coub_program.SetUniform("view_pos", view_pos);
        ...
        for (int i = 0; i < 6; i++) {
            ...
            float4x4 move = transpose(translate4x4(make_float3(3.5 * cos(phasa + cur_time * rot_rate), 3.5 * sin(phasa + cur_time * rot_rate), 7 * sin(phasa + cur_time * rot_rate))));
            ...
            triangles_program.SetUniform("light_pos", light_pos);
            triangles_program.SetUniform("view_pos", view_pos);
        ...
        float4x4 model = transpose(translate4x4(triangles_pos[1]));
        float rot_rate = 0.5;
        float4x4 move = transpose(translate4x4(make_float3(0.0, 0.0, 5 * sin(cur_time * rot_rate))));
        model = mul(rotate_Z_4x4(cur_time * rot_rate), model);
        model = mul(model, move);
        triangles_program.SetUniform("model", model);
        ...
        model = transpose(translate4x4(triangles_pos[1]));
        rot_rate = 0.5;
        move = transpose(translate4x4(make_float3(0.0, 0.0, 5 * sin(cur_time * rot_rate))));
        model = mul(rotate_Z_4x4(cur_time * rot_rate), model);
        model = mul(model, move);
        triangles_program.SetUniform("model", model);
        ...
        // triangles_program.SetUniform("projection", projection);
        // glBindVertexArray(triangles_VAO);
        // model = transpose(translate4x4(light_pos));
        // triangles_program.SetUniform("model", model);
        // triangles_program.SetUniform("color", colors[5]);
        // triangles_program.SetUniform("light_pos", light_pos);
        // triangles_program.SetUniform("view_pos", view_pos);
